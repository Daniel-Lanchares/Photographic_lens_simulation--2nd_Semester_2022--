# -*- coding: utf-8 -*-
"""
Created on Mon Mar  7 08:08:40 2022
...
def simulacion(sist, haces, ejes, j):
    ...
    for Obj in sist: 
        if type(Obj) == Diafragma: 
            Obj.pos = Planoi -fi
            ejes.text( round(di + sist[-1].pos)-20, -45,f'Número f: {round(fi/(2*(Obj.h)), 1)}')
    Diff.append(Planoi)
    ...
    ejes2.plot((di + sist[-1].pos)*np.ones(20), y_planos, 'r', label='Plano Focal Imagen')
    ejes2.plot((pos_sensor)*np.ones(20),y_planos*0.8 , 'k')
    ejes2.plot((pos_sensor)*np.ones(20),y_planos2 , 'tab:orange', label='Sensor')
    ejes2.plot(np.linspace((di + sist[-1].pos - fi),(di + sist[-1].pos), 20), -35**np.ones(20), 'k' )
    
    ejes2.plot(x, m*x+n, 'r--')
    ejes2.plot(x, -m*x-n, 'r--')
    
    ejes2.set_xlim(2110,2123)
    ejes2.set_ylim(-5,12.5)
    ejes2.set_title('Zoom en sensor (enfoque: $\infty$mm)')
    #plt.show()
...
pos=2000
focus1 = 0 #Infinito
#focus1 = -2.1985 #2 metros
#focus1 = -2.56 #1.720 metros (Enfoque mínimo)
focus = 0
n1 = 1.713
n2 = 1.64831
H = np.linspace(19, 19, nt)
...
fig, axs = plt.subplots(1,2,figsize=(16,9), gridspec_kw={'width_ratios': [7, 3]})
ejes = axs[0]
ejes2 = axs[1]
...
for i in  [90, ]:# [25, 42, 90]:  #np.linspace(25, 99, 74, dtype=int):#range(25, 100, 1): #[25,]:
    ...
    sist = [#Grupo 1
            ...
            Diafragma(pos=2025.5, y= 16,#16 para f2.8, 2.06 para f22, 8.1 para f5.6
                      ymax=22),
    ...
    H1 = Haz([0,24], np.linspace(-0.01,-0.005,3), config={
        'color':'tab:cyan'})
    ...
    H4 = Haz([280,-206], np.linspace(0.122,0.129,3), config={
        'color':'tab:purple'})
